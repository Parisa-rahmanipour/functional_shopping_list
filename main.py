...
def write_users_info(info_file:__file__, username: str, name: str, password: str, address: str, phone: str):
    """
    this func get users info as arg and write the infos as nested dict in json file
    Parameters
    ----------
    info_file:__file__ :
...
def check_user_password(info_file:__file__, username: str, password: str) -> bool: 
    """
...
def edit_user_info(info_file:__file__, username: str, name: str, password:str, address:str, phone:str):
    """
    this func read's user's info, delete the dicts and replace new dict by new info
    Parameters
    ----------
    info_file:__file__ :
        json file containing user's info
...
def get_categories(info_file:__file__, category:str):
    """
    this func reads json file containing products info and check product's category
...
clear_screen()
while True:
    registration_mode=input('have you registered in our site?:[YES/NO] ').upper()
    clear_screen()
    match registration_mode:
        case "NO":
            ...
            write_users_info('user_info.json', username, full_name, password, address, phone_number)
            logger.info(f'{username} registered')
            break
        case "YES":
            username=input(msg.enter_username)
            password=input(msg.enter_password)
            logged_in=check_user_password('user_info.json', username, password)
                ...
                while True:
                    ...
                    match info_edit_type:
                        case "1":
                            ...
                            username,password, full_name, address, phone_number=get_users_info('user_info.json', username)
                            username=new_username
                        case "2":
                            while True:
                                new_password=input(msg.enter_n_password)
                                new_password_confirm=input(msg.enter_n_pass_confirm)
                        ...
                        case _:
                            ...
                            print(msg.invalid_command)
                            msg.edit_info_help()
                    edit_user_info("user_info.json", username, full_name, password, address, phone_number)
            else:
                continue
        case "2":
...
while True:
    ...
    if new_item in shopping_list:
        ...
        match new_item:
            ...
            case _:
                available_products=check_available_products('shopping_list_data.json')
...
for length in range(len(shopping_list)):
    quantity = int(input(f'how many boxes of {shopping_list[length]} you want?: '))
    clear_screen()
